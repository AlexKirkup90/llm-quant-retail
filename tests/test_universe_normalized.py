...
def test_prepare_backtest_universe_handles_non_standard_schema(monkeypatch):
    df = pd.DataFrame(
        {
            "Ticker": ["spy", "msft", ""],
            "Security": ["SPDR", "Microsoft", ""],
            "Industry": ["ETF", "Technology", "Utilities"],
        }
    )
